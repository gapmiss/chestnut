...
`spctl -a -vv` reports `rejected` for this app and always will — it is ad-hoc signed, not notarized, which is the whole point. That verdict describes the signature, not what happens when the user double-clicks, so it is not a Gatekeeper prediction either. **Neither signal justifies telling the maintainer to run `xattr -dr` after an upgrade**; they have corrected this three times — twice from the quarantine attribute, once from `spctl`. The only thing that settles whether an upgrade prompts is launching the app. This paragraph existed and was missed on the third occasion, which is why the tripwire index in `CLAUDE.md` now carries a Distribution entry pointing here.
